...
uint16_t half_from_float( uint32_t i );
uint32_t half_to_float(uint16_t y);
void     half_to_float_x8( const uint16_t* restrict const h, uint32_t* restrict const f );
void     half_from_float_x8( const uint32_t* restrict const f, uint16_t* restrict const h );
uint16_t ilm_half_from_float( uint32_t i );
uint32_t ilm_half_to_float(uint16_t y);
...
ilm_test_conversion( void ) 
{
    ...
    for ( test_ndx = 0; test_ndx < mixed_test_count; test_ndx++ )
    {
      ...
      if ( test_type == 0 )
      {
        ...
        test_in  = float_test_vector_special[i];
        test_f16 = ilm_half_from_float( test_in );
      ...
      else
      {
        ...
        uint16_t test_f16          = ilm_half_from_float( test_in );
        uint32_t test_f32          = ilm_half_to_float( test_f16 );
        uint16_t test_f16_reversed = ilm_half_from_float( test_f32 );
        uint16_t test_diff         = test_f16_reversed - test_f16;
...
{   
    ...
    total_time = ((end.tv_sec-start.tv_sec) * 1.e6) + (end.tv_usec-start.tv_usec);
    double avg_time = total_time / (1.e6 * RUN_COUNT ); 

    printf("avg_time:%g sec\n",avg_time);
}
