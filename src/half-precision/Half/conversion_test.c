...
unsigned int ilm_half_to_float(unsigned short y);
...
ilm_test_conversion( void ) 
{
    ...
    for ( i=0; i < float_test_vector_count; i++ )
    {
      ...
      test_in  = float_test_vector_med[i];
      test_f16 = ilm_half_from_float( test_in );
      test_f32 = ilm_half_to_float( test_f16 );
    ...
    for ( i=0; i < float_test_vector_count; i++ )
    {
      ...
      uint16_t h_expected = ilm_half_from_float( test_f32 );
      uint16_t h_received = test_f16;
...
{   
    ...
    double avg_time = total_time / (1.e6 * TEST_RUN_COUNT ); 
    ...
    double ilm_avg_time = total_time / (1.e6 * TEST_RUN_COUNT ); 

    printf("ilm_avg_time:%g sec\n",ilm_avg_time);
    printf("avg improvement: %g percent\n",100.0*(ilm_avg_time-avg_time)/ilm_avg_time);
...
#if 1
uint16_t
ilm_half_from_float( uint32_t i )
{
    //
...
unsigned int
ilm_half_to_float(unsigned short y)
{
