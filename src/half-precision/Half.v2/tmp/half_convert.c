...
half_to_float( uint16_t h )
{
  ...
  // MJA - 
  // uint32_t  w = (uint32_t)h;   // OK
  // uint32_t  s =  (w & 0x8000); // OK, but needs to be shifted left 16
  // if (s != 0x8000) s = 0x0;    // Why? This is not needed.
  // uint32_t  e = ((w & 0x7c00) >> 10 ) - 15 + 127; // OK
  // uint32_t  m =  (w & 0x03ff) << 13;              // OK
  ...
  else if ( he == 0x0000 )
  {
    fm = 0;
    fe = 0;
    // TODO: Convert very small half numbers (denorms) to float
  }
